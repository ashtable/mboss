...
7. **Cut the next branch, and tell the repository its own name.** The new version is the requested version resolved above, if any; otherwise use a patch bump of the branch from step 1 (`mcp-server-v0.0.3` →
...
8. **Open the next branch's PR, so its tip has CI.**
   ...
   the `codex-release-root.md` workflow refuses to pin them.
