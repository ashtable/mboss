---
description: Release the mboss-e2e-tests submodule — merge its version branch into main, cut the next version branch, and bump the superproject pointer
argument-hint: "[next version, e.g. 0.1.0 — defaults to a patch bump]"
allowed-tools: Bash(git:*), Bash(gh:*), Read, Edit
---

Release the `mboss-e2e-tests` submodule. Run every git command with `-C mboss-e2e-tests`
...
1. **Resolve the current version branch.**
   Read the checked-out branch: `git -C mboss-e2e-tests rev-parse --abbrev-ref HEAD`.
   It must match `e2e-tests-vX.Y.Z`. If it does not, find the latest with
   `git -C mboss-e2e-tests branch -a --list '*e2e-tests-v[0-9]*' --sort=-v:refname | head -1`,
   check it out, and say which branch you picked. If no such branch exists, stop and report.

2. **Commit all changes on that branch.**
   `git -C mboss-e2e-tests add -A` then commit. Write a real message summarizing the actual
   diff — do not use a placeholder. If the tree is already clean, skip this step and note it.

3. **Push the branch.** `git -C mboss-e2e-tests push -u origin <branch>`

4. **Open the PR.** `gh pr create --repo ashtable/mboss-e2e-tests --base main --head <branch>`
...
6. **Sync main.** `git -C mboss-e2e-tests checkout main && git -C mboss-e2e-tests pull --ff-only`

7. **Cut the next branch.** The new version is the requested version resolved above, if any; otherwise use a patch bump of
   the branch from step 1 (`e2e-tests-v0.0.3` → `e2e-tests-v0.0.4`). Create it off the freshly
   merged main and push:
   `git -C mboss-e2e-tests checkout -b e2e-tests-v<next>` then
   `git -C mboss-e2e-tests push -u origin e2e-tests-v<next>`

8. **Bump the superproject.** In `/Users/ash/code/mboss`:
   - `git config -f .gitmodules submodule.mboss-e2e-tests.branch e2e-tests-v<next>`
   - `git submodule sync --quiet mboss-e2e-tests`
   - `git add .gitmodules mboss-e2e-tests`
   - Commit with a message naming the released and new versions.

   Stage only `.gitmodules` and `mboss-e2e-tests` — never `-A`, so unrelated superproject
