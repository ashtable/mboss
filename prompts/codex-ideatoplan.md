...
implementation plan they can hand to Codex.
...
says what to design, stop and ask before doing anything else.
...
- **Clear this command's own scratch files first.** A stale `stepN-*.md` from a previous run is
  ...
  **Delete only these paths.** The workflow in `./prompts/codex-plantocode.md` also writes
  `stepN-*.md` files into `./scratch/`, and its names differ from these, so a blanket
  `rm ./scratch/step*` would destroy its output. Leave every other file in `./scratch/` alone.
...
Launch seven `terra-engineer` subagents **concurrently**, one per directory. Each explores and
...
These two reviews are independent — launch both `terra-engineer` subagents **concurrently** in
