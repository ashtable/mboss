...
Optional custom-prompt arguments: `$ARGUMENTS`

If the preceding value still begins with a dollar sign, treat it as unset. Treat the user's
current message, including any attached or `@`-mentioned files, as the request. If neither source
says which plan task(s) to implement, stop and ask before doing anything else.
...
- **Create `./scratch/` if it doesn't exist** before launching any subagent. The design docs
  ...
  stop and ask the user to run the workflow in `./prompts/codex-ideatoplan.md` first.
- **Clear this command's own scratch files first.** A stale `stepN-*.md` from a previous run is
  ...
  **Delete only these paths.** The workflow in `./prompts/codex-ideatoplan.md` also writes
  `stepN-*.md` files into `./scratch/`, and its names differ from these, so a blanket
  `rm ./scratch/step*` would destroy the design research this command depends on. Leave every
  other file in `./scratch/` alone.
...
- **Confirm before releasing; version branches only.** Steps 5, 6, 11, and 12 commit and push,
  ...
  goes to the repo's current `*-vX.Y.Z` version branch, never directly to `main`; the matching
  `./prompts/codex-release-*.md` workflows are the only path to `main`. Stop and get the user's
  explicit go-ahead before starting (Step 13).
...
Launch a new `terra-engineer` subagent to:
...
Launch a new `terra-engineer` subagent (give it the user's request verbatim) to summarize each
...
Launch seven `terra-engineer` subagents **concurrently**, one per directory. Each reviews its
...
Launch a new `sol-engineer` subagent to:
...
- Assemble an overall test-driven development plan using Playwright and unit tests as
  ...
  available.)
- Note every cross-repo dependency across the Git submodules: the
  `./prompts/codex-release-*.md` workflows run only at the very end, so mid-run the plan must
  sometimes commit code to one repo and update submodule pointers to it before the next piece
  of code can be written or a test can pass.
...
These two reviews are independent — launch both `terra-engineer` subagents **concurrently**
...
a new `terra-engineer` subagent to:

- Read `./scratch/step6-commits.md` and `./scratch/step12-commits.md` to determine which
  submodules changed.
- For each updated submodule, read and follow its corresponding release prompt in `./prompts/`:
  `codex-release-database.md`, `codex-release-zod.md`, `codex-release-api.md`,
  `codex-release-dbos.md`, `codex-release-vscode.md`, or `codex-release-web.md`.
- Finally, read and follow `./prompts/codex-release-root.md` to release the root project.
...
and confirm the `codex-release-root.md` workflow ran (or that the user declined the release).
Call out anything unfinished, skipped, or unverified.
