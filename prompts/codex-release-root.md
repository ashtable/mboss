...
- **Uncommitted changes?** `git -C <path> status --porcelain` must be empty for every
  ...
  the work would simply be left behind; that submodule's corresponding release workflow
  (`codex-release-web.md`, `codex-release-api.md`, `codex-release-dbos.md`, …) should run first.
...
- **Nested pin parity?** For every submodule `mboss-e2e-tests` nests, the commit it
  ...
  no `codex-release-<repo>.md` workflow touches a nested pin, so that edit is always by hand and
...
- **CI green on every pin?** For each submodule this release records, the newest CI run
  ...
  protected, so a `codex-release-<repo>.md` workflow can merge a version branch before its own CI
  has even started.
